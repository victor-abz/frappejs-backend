...
const { indicators: indicatorColor } = require('../utils/indicators');
...
module.exports = class BaseMeta extends BaseDocument {
	...
	processFields() {
		// add name field
		if (
			!this.fields.find((df) => df.fieldname === 'name') &&
			!this.isSingle
		) {
			this.fields = [
				{
		...
		if (this._hasFormula === undefined) {
			...
			if (this.getFormulaFields().length) {
				...
				for (let tablefield of this.getTableFields()) {
					if (
						frappe.getMeta(tablefield.childtype).getFormulaFields()
							.length
					) {
						this._hasFormula = true;
						break;
	...
	getKeywordFields() {
		if (!this._keywordFields) {
			...
			if (
				!(
					this._keywordFields &&
					this._keywordFields.length &&
					this.fields
				)
			) {
				this._keywordFields = this.fields
					.filter(
						(field) => field.fieldtype !== 'Table' && field.required
					)
	...
	getQuickEditFields() {
		if (this.quickEditFields) {
			return this.quickEditFields.map((fieldname) =>
